...
export const getBranchById = async (branchId: number) => {
  const { data } = await apiClient.get<RestaurantBranch>(`/branch/${branchId}`);
  return data;
};
...
export const getBranchesForRestaurant = async (restaurantId: number) => {
  const { data } = await apiClient.get<RestaurantBranch[]>(`/branches/${restaurantId}`);
  return data;
};

// Get all branches (admin only)
export const getAllBranches = async () => {
  const { data } = await apiClient.get<RestaurantBranch[]>(`/branches/all`);
  return data;
};
